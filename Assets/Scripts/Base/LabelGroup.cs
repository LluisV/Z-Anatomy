...
public class LabelGroup : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        SetText(gameObject.name.Replace(".j", "").Replace(".i", ""));
        try
        {
            Transform maxPoint = transform.parent.Find(gameObject.name.Replace(".gLabel", ".j")).Find("maxPoint");
            Transform minPoint = transform.parent.Find(gameObject.name.Replace(".gLabel", ".j")).Find("minPoint");
            if(maxPoint == null && minPoint == null)
            {
                maxPoint = transform.parent.Find(gameObject.name.Replace(".gLabel", ".i")).Find("maxPoint");
                minPoint = transform.parent.Find(gameObject.name.Replace(".gLabel", ".i")).Find("minPoint");
            }
        ...
        foreach (Transform child in transform.parent)
        {
            foreach (Transform label in child.transform)
            {
                if (label.name.Contains(".gLabel"))
                {
                    ...
                    child.Find(label.name.Replace(".gLabel", ".j")).gameObject.SetActive(true);
                }
