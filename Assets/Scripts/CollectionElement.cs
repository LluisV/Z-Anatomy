...
public class CollectionElement : MonoBehaviour
{
    ...
    private void ShortClick()
    {
        ...
        {
            //Disable everything
            StaticMethods.SetActiveRecursively(MeshManagement.instance.globalParent.transform, false);

            //Get all objects
            List<NameAndDescription> gameObjects = MeshManagement.instance.bodyPartsRenderers
            ...
            foreach (string item in collectionData.elements)
            {
                ...
                if (found != null)
                {
                    StaticMethods.SetActiveParentsRecursively(found.transform, true);
