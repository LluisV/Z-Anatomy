...
public class Lexicon : MonoBehaviour
{
        ...
        foreach (Transform child in GlobalVariables.Instance.globalParent.transform)
        {
            ...
            if (!child.CompareTag("Debug"))
            {
                ...
                if (layersSlider != null)
                {
                    ...
                    layersSlider.transform.localScale = new Vector2(.15f, .15f);
                    layersSlider.GetComponent<RectTransform>().anchoredPosition = new Vector2(rt.anchoredPosition.x - 20, 0);          
                }
        ...
        foreach (Transform child in childs)
        {
            if (!child.name.Contains(".j") && !child.name.Contains(".i") && !child.CompareTag("Insertions"))
            {
                ...
                if (!firstInstance)
                {
                    ...
                    isVisibleScript.lexiconElement = script;
                    Visibility parentIsVisibleScript = parentScript.element.GetComponent<Visibility>();
                ...
                else
                {
                    ...
                    lineRt.anchoredPosition = new Vector2(-5, 0);
                    lineRt.SetWidth(10);
                    lineRt.SetHeight(1);
    ...
    public void ExpandRecursively()
    {
        ...
        {
            LexiconElement lexElement = HierarchyBar.Instance.hierarchyObjects[i].GetComponent<Visibility>().lexiconElement;
            if(lexElement == null)
            {
    ...
    public void UpdateTreeViewCheckboxes(bool updateSliders = true)
    {
        foreach (var element in elements)
        {
            ...
            var isVisibleScript = elementScript.element.GetComponent<Visibility>();
    ...
    {
        return parent.GetComponentsInChildren<BodyPart>(true).Where(it => it.isActiveAndEnabled).Count() > 0;
    }
    ...
    public void UpdateSelected()
    {
        foreach (var element in elements)
        {
            ...
            if (elementScript != null)
            {
                var script = elementScript.element.GetComponent<Visibility>();
                if(script != null)
                {
