...
public class NerveGroups : MonoBehaviour
{
    ...
    Dictionary<string, List<Visibility>[]> nerves = new Dictionary<string, List<Visibility>[]>();
    Dictionary<string, List<Visibility>[]> muscles = new Dictionary<string, List<Visibility>[]>();
    private void Awake()
    {
        Instance = this;
        ...
        var nervesList = GlobalVariables.Instance.globalParent.GetComponentsInChildren<Visibility>(true).Where(it => it.CompareTag("Nervous"));
        ...
        foreach (var nerve in nervesList)
        {
            if (!nerves.ContainsKey(nerve.nameScript.originalName.RemoveSuffix()))
                nerves.Add(nerve.nameScript.originalName.RemoveSuffix(), new List<Visibility>[2]);


            if (nerves[nerve.nameScript.originalName.RemoveSuffix()][0] == null)
                nerves[nerve.nameScript.originalName.RemoveSuffix()][0] = new List<Visibility>();

            if (nerves[nerve.nameScript.originalName.RemoveSuffix()][1] == null)
                nerves[nerve.nameScript.originalName.RemoveSuffix()][1] = new List<Visibility>();
        ...
        foreach (var muscle in musclesList)
        {
            if (!muscles.ContainsKey(muscle.nameScript.originalName.RemoveSuffix()))
                muscles.Add(muscle.nameScript.originalName.RemoveSuffix(), new List<Visibility>[2]);


            if (muscles[muscle.nameScript.originalName.RemoveSuffix()][0] == null)
                muscles[muscle.nameScript.originalName.RemoveSuffix()][0] = new List<Visibility>();

            if (muscles[muscle.nameScript.originalName.RemoveSuffix()][1] == null)
                muscles[muscle.nameScript.originalName.RemoveSuffix()][1] = new List<Visibility>();
    ...
    public void GetNerveMuscles(Visibility nerve)
    {
            ...
            if (group.Contains(nerveName))
            {
                for (int i = 0; i < group.Count; i++)
                {
                    ...
                    if (isRight && muscles[group[i]][0] != null)
                        nerve.nerveMuscles.AddRange(muscles[group[i]][0].ConvertAll(it => it.GetComponent<BodyPart>()));
                    else if (muscles[group[i]][1] != null)
                        nerve.nerveMuscles.AddRange(muscles[group[i]][1].ConvertAll(it => it.GetComponent<BodyPart>()));
    ...
    public void GetMuscleNerves(Visibility muscle)
    {
            ...
            if (group.Contains(muscleName))
            {
                for (int i = 0; i < group.Count; i++)
                {
                    ...
                    if (isRight && nerves[group[i]][0] != null)
                        muscle.muscleNerves.AddRange(nerves[group[i]][0].ConvertAll(it => it.GetComponent<BodyPart>()));
                    else if (nerves[group[i]][1] != null)
                        muscle.muscleNerves.AddRange(nerves[group[i]][1].ConvertAll(it => it.GetComponent<BodyPart>()));
