...
public class ContextualMenu : MonoBehaviour
{
    ...
    public void ShowLabelsClick()
    {
        ...
        {
            contextObject.GetComponent<Visibility>().ShowLabels(true);
        ...
        if (contextObject == null || SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
        {
            foreach (var item in SelectedObjectsManagement.Instance.selectedObjects)
            {
                item.GetComponent<Visibility>().ShowLabels(true);
            }
        }
    ...
    {
        if (contextObject != null && !SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
            contextObject.GetComponent<Visibility>().HideLabels(true);

        if (contextObject == null || SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
        {
            foreach (var item in SelectedObjectsManagement.Instance.selectedObjects)
            {
                item.GetComponent<Visibility>().HideLabels(true);
            }
        }
        Lexicon.Instance.UpdateTreeViewCheckboxes();
    ...
    { 
        ...
        if(contextObject != null && !SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
            shown = contextObject.GetComponent<Visibility>().ShowInsertions();

        if (contextObject == null || SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
        {
            foreach (var item in SelectedObjectsManagement.Instance.selectedObjects.ToList())
            {
                shown.AddRange(item.GetComponent<Visibility>().ShowInsertions());
            }
        }
        Lexicon.Instance.UpdateTreeViewCheckboxes();
    ...
    public void HideInsertionsClick()
    {
        ...
        if (contextObject != null && !SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
            hidden = contextObject.GetComponent<Visibility>().HideInsertions();

        if (contextObject == null || SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
        {
            foreach (var item in SelectedObjectsManagement.Instance.selectedObjects)
            {
                hidden.AddRange(item.GetComponent<Visibility>().HideInsertions());
            }
        }
        Lexicon.Instance.UpdateTreeViewCheckboxes();
    ...
    public void ShowMuscleClick()
    {
        List<GameObject> shown = new List<GameObject>(); ;
        if (contextObject != null && !SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
            shown = contextObject.GetComponent<Visibility>().ShowMuscle();

        if (contextObject == null || SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
        {
            foreach (var item in SelectedObjectsManagement.Instance.selectedObjects.ToList())
            {
                shown.AddRange(item.GetComponent<Visibility>().ShowMuscle());
            }
        }
    ...
    public void ShowMuscleNervesClick()
    {
        ...
        if (contextObject != null && !SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
            shown = contextObject.GetComponent<Visibility>().ShowMuscleNerves();

        if (contextObject == null || SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
        {
            foreach (var item in SelectedObjectsManagement.Instance.selectedObjects.ToList())
            {
                shown.AddRange(item.GetComponent<Visibility>().ShowMuscleNerves());
            }
        }
        Lexicon.Instance.UpdateTreeViewCheckboxes();
    ...
    public void ShowNerveMusclesClick()
    {
        ...
        if (contextObject != null && !SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
            shown = contextObject.GetComponent<Visibility>().ShowNerveMuscles();

        if (contextObject == null || SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
        {
            foreach (var item in SelectedObjectsManagement.Instance.selectedObjects.ToList())
            {
                shown.AddRange(item.GetComponent<Visibility>().ShowNerveMuscles());
            }
        }
        Lexicon.Instance.UpdateTreeViewCheckboxes();
    ...
    private bool ShowLabelsOn()
    {
        ...
        if (SelectedObjectsManagement.Instance.selectedObjects.Count == 0)
        {
            var visibility = contextObject.GetComponent<Visibility>();
        ...
        else
        {
            ...
            {
                var visibility = SelectedObjectsManagement.Instance.selectedObjects[0].GetComponent<Visibility>();
                return visibility.HasLabels() && !visibility.labelsOn;
            }
        }
    }
    ...
    private bool ResetPositionOn()
    {
        if (SelectedObjectsManagement.Instance.selectedObjects.Count == 0)
        {
            var bodyPart = contextObject.GetComponent<BodyPart>();
        ...
        else
        {
            if (contextObject == null || SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
            {
                foreach (var item in SelectedObjectsManagement.Instance.selectedObjects)
                {
                    var bodyPart = item.GetComponent<BodyPart>();
    ...
    private bool ShowInsertionsOn()
    {
        ...
        else
        {
            if (contextObject == null || SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
            {
                ...
                foreach (var item in SelectedObjectsManagement.Instance.selectedObjects)
                {
                    var visibility = item.GetComponent<Visibility>();
    ...
    private bool ShowInsertionMuscleOn()
    {
        if (SelectedObjectsManagement.Instance.selectedObjects.Count == 0)
        {
            var bodyPart = contextObject.GetComponent<Visibility>();
        ...
        else
        {
            if (contextObject == null || SelectedObjectsManagement.Instance.selectedObjects.Contains(contextObject))
            {
                foreach (var item in SelectedObjectsManagement.Instance.selectedObjects)
                {
                    var bodyPart = item.GetComponent<Visibility>();
                    if (bodyPart == null)
                        return false;
